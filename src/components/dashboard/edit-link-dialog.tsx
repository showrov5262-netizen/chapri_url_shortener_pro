...
import { Globe, Smartphone, Users, X, Bot, Lock, ShieldCheck } from "lucide-react";
...
export function EditLinkDialog({ link, isOpen, onOpenChange, onUpdateLink }: EditLinkDialogProps) {
    ...
    useEffect(() => {
        if (link && isOpen) {
            ...
            setCaptchaVerification(link.captchaVerification || false);
            ...
            setSpoof(link.spoof || { title: '', description: '', imageUrl: '' });
            setUseSpoof(!!link.spoof);

            // Populate advanced targeting
            ...
            // Check for available loading pages and CAPTCHA config
            try {
                ...
                if (captchaConfig) {
                    ...
                    setIsCaptchaConfigured(!!(parsed.siteKey && parsed.secretKey));
                } else {
        ...
        <div className="grid gap-6 py-4">
          ...
          <Accordion type="multiple" className="w-full">
            <AccordionItem value="redirection-options">
              ...
              <AccordionContent className="pt-4 space-y-6">
                 <div className="flex items-center justify-between rounded-lg border p-3 shadow-sm">
                    <div className="space-y-0.5"><Label>Frame-based Cloaking</Label><p className="text-xs text-muted-foreground">Masks the destination URL. Some sites may block this.</p></div>
                    <Switch checked={isCloaked} onCheckedChange={setIsCloaked} />
                  </div>
                <div className="rounded-lg border p-3 shadow-sm space-y-3">
                  <Label>Redirect Type</Label>
                  <Select value={redirectType} onValueChange={(value: '301' | '302') => setRedirectType(value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="301">301 Permanent</SelectItem><SelectItem value="302">302 Temporary</SelectItem></SelectContent></Select>
                </div>
                <div className="rounded-lg border p-3 shadow-sm space-y-3">
                  <div className="flex items-center justify-between"><div className="space-y-0.5"><Label>Meta Refresh Redirect</Label></div><Switch checked={useMetaRefresh} onCheckedChange={setUseMetaRefresh} /></div>
                   {useMetaRefresh && (<div className="space-y-4 pt-4"><div className="grid gap-2"><Label htmlFor="edit-redirect-delay" className="text-xs">Redirect delay (seconds)</Label><Input id="edit-redirect-delay" type="number" value={metaRefreshDelay ?? ''} onChange={(e) => setMetaRefreshDelay(parseInt(e.target.value, 10) || 0)} /></div></div>)}
                </div>
                <div className="rounded-lg border p-3 shadow-sm space-y-3">
                    <div className="flex items-center justify-between"><div className="space-y-0.5"><Label>Spoof Social Media Preview</Label></div><Switch checked={useSpoof} onCheckedChange={setUseSpoof} /></div>
                    {useSpoof && (<div className="space-y-4 pt-2"><div className="grid gap-2"><Label htmlFor="edit-spoof-title" className="text-xs">Spoofed Title</Label><Input id="edit-spoof-title" value={spoof?.title || ''} onChange={(e) => setSpoof(s => ({...s!, title: e.target.value}))} /></div><div className="grid gap-2"><Label htmlFor="edit-spoof-description" className="text-xs">Spoofed Description</Label><Textarea id="edit-spoof-description" value={spoof?.description || ''} onChange={(e) => setSpoof(s => ({...s!, description: e.target.value}))} /></div><div className="grid gap-2"><Label className="text-xs" htmlFor="edit-spoof-image-url">Spoofed Thumbnail URL</Label><Input id="edit-spoof-image-url" value={spoof?.imageUrl || ''} onChange={(e) => setSpoof(s => ({...s!, imageUrl: e.target.value}))} /></div></div>)}
                  </div>
              </AccordionContent>
            </AccordionItem>
            <AccordionItem value="security-options">
              ...
              <AccordionContent className="pt-4 space-y-6">
                  <div className="rounded-lg border p-3 shadow-sm space-y-3">
                    <div className="flex items-center justify-between"><div className="space-y-0.5"><Label>Password Protection</Label></div><Switch checked={usePassword} onCheckedChange={setUsePassword} /></div>
                    {usePassword && (<div className="grid gap-2 pt-2"><Label htmlFor="edit-link-password" className="text-xs">Password</Label><Input id="edit-link-password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} /></div>)}
                  </div>
                   <div className="rounded-lg border p-3 shadow-sm"><div className="flex items-center justify-between"><div className="space-y-0.5"><Label className="flex items-center gap-2" htmlFor="edit-base64-encoding"><Lock className="h-4 w-4" />Encode Base64</Label></div><Switch id="edit-base64-encoding" checked={useBase64Encoding} onCheckedChange={setUseBase64Encoding} /></div></div>
                   <TooltipProvider><Tooltip><TooltipTrigger asChild><div className="rounded-lg border p-3 shadow-sm has-[[data-disabled=true]]:opacity-50 has-[[data-disabled=true]]:cursor-not-allowed"><div className="flex items-center justify-between"><div className="space-y-0.5"><Label className="flex items-center gap-2"><ShieldCheck className="h-4 w-4" />CAPTCHA Verification</Label></div><Switch checked={captchaVerification} onCheckedChange={setCaptchaVerification} disabled={!isCaptchaConfigured} data-disabled={!isCaptchaConfigured} /></div></div></TooltipTrigger>{!isCaptchaConfigured && (<TooltipContent><p>CAPTCHA is not configured. Please add keys in settings.</p></TooltipContent>)}</Tooltip></TooltipProvider>
                  <div className="rounded-lg border p-3 shadow-sm space-y-3">
                    <div className="flex items-center justify-between"><div className="space-y-0.5"><Label>Link Expiration</Label></div><Switch checked={useExpiration} onCheckedChange={setUseExpiration} /></div>
                    {useExpiration && (<div className="grid grid-cols-2 gap-4 pt-2"><div className="grid gap-2"><Label htmlFor="edit-max-clicks" className="text-xs">Max Clicks</Label><Input id="edit-max-clicks" type="number" value={maxClicks ?? ''} onChange={(e) => setMaxClicks(parseInt(e.target.value, 10))} /></div><div className="grid gap-2"><Label htmlFor="edit-expiration-date" className="text-xs">Expiration Date</Label><Popover><PopoverTrigger asChild><Button variant="outline" className="font-normal justify-start">{expiresAt ? format(expiresAt, 'PPP') : <span>Pick a date</span>}</Button></PopoverTrigger><PopoverContent className="w-auto p-0"><Calendar mode="single" selected={expiresAt} onSelect={setExpiresAt} initialFocus /></PopoverContent></Popover></div></div>)}
                  </div>
              </AccordionContent>
            </AccordionItem>
            <AccordionItem value="targeting-options">
              ...
              <AccordionContent className="pt-4 space-y-6">
                <div className="rounded-lg border p-3 shadow-sm space-y-3">
                  <div className="flex items-center justify-between"><div className="space-y-0.5"><Label>Geo-Targeting</Label></div><Switch checked={useGeoTargeting} onCheckedChange={setUseGeoTargeting} /></div>
                ...
                <div className="rounded-lg border p-3 shadow-sm space-y-3">
                  <div className="flex items-center justify-between"><div className="space-y-0.5"><Label>Device Targeting</Label></div><Switch checked={useDeviceTargeting} onCheckedChange={setUseDeviceTargeting} /></div>
                ...
                <div className="rounded-lg border p-3 shadow-sm space-y-3">
                  <div className="flex items-center justify-between"><div className="space-y-0.5"><Label>A/B Testing (Rotator)</Label></div><Switch checked={useABTesting} onCheckedChange={setUseABTesting} /></div>
                ...
                <div className="rounded-lg border p-3 shadow-sm space-y-3">
                  <div className="flex items-center justify-between"><div className="space-y-0.5"><Label>Retargeting Pixels</Label></div><Switch checked={usePixels} onCheckedChange={setUsePixels} /></div>
            ...
            <AccordionItem value="loading-page-options" disabled={!useMetaRefresh}>
                ...
                <AccordionContent className="pt-4 space-y-6">
                    <div className="rounded-lg border p-3 shadow-sm space-y-3">
                        <div className="flex items-center justify-between"><div className="space-y-0.5"><Label>Configure Loading Page</Label></div><Switch checked={useLoadingPageOverride} onCheckedChange={setUseLoadingPageOverride} /></div>
                        {useLoadingPageOverride && (<div className="space-y-4 pt-4 border-t"><RadioGroup value={loadingPageConfig.mode} onValueChange={(value: 'global' | 'random' | 'specific') => setLoadingPageConfig(prev => ({ ...prev, mode: value, useGlobal: value === 'global' }))}><div className="flex items-center space-x-2"><RadioGroupItem value="global" id="edit-global" /><Label htmlFor="edit-global" className="font-normal">Use Global Setting</Label></div><div className="flex items-center space-x-2"><RadioGroupItem value="random" id="edit-random" /><Label htmlFor="edit-random" className="font-normal">Show a random page</Label></div><div className="flex items-center space-x-2"><RadioGroupItem value="specific" id="edit-specific" /><Label htmlFor="edit-specific" className="font-normal">Use a specific page</Label></div></RadioGroup>{loadingPageConfig.mode === 'specific' && (<div className="grid gap-2 pt-2"><Label htmlFor="edit-select-page" className="text-xs">Select Page</Label><Select value={loadingPageConfig.selectedPageId ?? ""} onValueChange={(value) => setLoadingPageConfig(prev => ({...prev, selectedPageId: value}))}><SelectTrigger id="edit-select-page"><SelectValue placeholder="Select a loading page" /></SelectTrigger><SelectContent>{availableLoadingPages.map(page => (<SelectItem key={page.id} value={page.id}>{page.name}</SelectItem>))}</SelectContent></Select></div>)}</div>)}
                    </div>
                </AccordionContent>
            </AccordionItem>
          </Accordion>
